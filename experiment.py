...
class RetrofitExperiment(Experiment):
    ...
    def compute_loss_and_update_metrics(self, word_rep_pos_1: torch.Tensor, word_rep_pos_2: torch.Tensor,
        ...
        self.metric_averages.update(f'{metrics_key}/pos_pair_dist_mean', neg_pair_distance.item())
        word_rep_neg = torch.cat([word_rep_neg_1, word_rep_neg_2], dim=0)
        word_rep_neg_norm = torch.norm(word_rep_neg, p=2, dim=1).mean()
        self.metric_averages.update(f'{metrics_key}/neg_pair_dist_mean', word_rep_neg_norm.item())
        avg_norm = (word_rep_pos_norm + word_rep_neg_norm) / 2.0
        self.metric_averages.update(f'{metrics_key}/avg_pair_dist_mean', avg_norm.item())
        return loss
